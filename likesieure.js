...
(async () => {
    try {
        ...
        async function Dungkon() {
            if (key) {
                console.log('===== Đang tiền hành tìm job likesieure =====');
                var time = moment.tz("Asia/Ho_Chi_Minh").format("HH:mm:ss");

                var results = await axios.post(
                    `https://traodoisub.com/api/?fields=likegiare&access_token=${token}`,
                );
                ...
                for (let i = 0; i < results.data.length; i++) {
                    try {
                        ...
                        var rp = await axios.post(
                            `https://traodoisub.com/api/coin/?type=likesieure&id=${results.data[i].id}&access_token=${token}`,
                        );
