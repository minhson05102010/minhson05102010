...
(async () => {
    try {
        ...
        async function Dungkon() {
              console.log('===== Đang tiền hành tìm job follow =====');
                var time = moment.tz("Asia/Ho_Chi_Minh").format("HH:mm:ss");
                        var results = await axios.post(
                    `https://traodoisub.com/api/?fields=follow&published=0&access_token=${token}`,
                );
                ...
                for (let i = 0; i < results.data.length; i++) {
                    try {
                        ...
                        var data = await axios.post(
                            `https://graph.facebook.com/${results.data[i].id}/subscribers?method=POST&access_token=${tokenfb}`,
                        );

                        var rp = await axios.post(
                            `https://traodoisub.com/api/coin/?type=follow&id=${results.data[i].id}&access_token=${token}`,
                        );
